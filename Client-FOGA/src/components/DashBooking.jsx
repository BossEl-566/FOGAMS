...
export default function DashBooking() {
    ...
    const [bookings, setBookings] = useState([]); // Changed from availableSlots to bookings
  ...
  useEffect(() => {
    const fetchAvailability = async () => {
      try {
        ...
        // Extract the availableSlots from each booking
        const allSlots = data.flatMap(booking => booking.availableSlots || []);

        setBookings(data); // Store the full bookings data
        console.log(data);
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (refreshResponse.ok) {
        setAvailableSlots(await refreshResponse.json());
      }
    } catch (error) {
      toast.error(error.message);
    } finally {
  ...
  const handleBookSlot = async (slotId, parentSlot) => {
    ...
    try {
      setIsLoading(true);
      const response = await fetch(`/api/book/book-appointment/${bookingId}/${slotId}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
