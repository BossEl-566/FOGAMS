...
export default function DashMainPageDBM() {
  ...
  useEffect(() => {
    const fetchDailyBibleMessage = async () => {
      try {
        ...
        toast.error('Failed to load daily Bible messages') // Toast for error
      }
    }
  ...
  const handleShowMore = async () => {
    ...
    try {
      ...
      console.log(error.message)
      toast.error('Failed to load more messages') // Toast for error
    }
  }
  ...
  const handleDeleteMessage = async () => {
    ...
    try {
      ...
      const data = await res.json()
      if (!res.ok) {
        toast.error('Failed to delete message') // Toast for error
        console.log(data.message)
      } else {
        setDailyBibleMessage((prev) => prev.filter((message) => message._id !== deleteId))
        toast.success('Message deleted successfully') // Toast for success
      }
    } catch (error) {
      console.log(error.message)
      toast.error('Error occurred while deleting message') // Toast for error
    }
  }
                ...
                <Table.Row className='bg-white dark:border-gray-700 dark:bg-gray-800'>
                  ...
                  <Table.Cell>
                    <Link to={`/daily-bible-message/${message.slug}`}>
                      <img src={message.image} alt={message.title} className='w-20 h-20 object-cover bg-gray-500' />
                    </Link>
                  </Table.Cell>
                  <Table.Cell>
